...
mod tests {
    ...
    fn surge_32_batch_e2e() {
        ...
        // Generate a list of NUM_OPS operations. Each op is a SHA‑256 hash of a randomly
        // generated message string.
        let ops: Vec<MessageOp> = (0..NUM_OPS)
            .map(|_| {
                // Create a dummy message (here using a random number).
                let msg = format!("random message {}", rng.next_u32());
                ...
                MessageOp { hash: hash_arr }
            })
            .collect();

        // Preprocess and set up generators (as defined in the Surge framework).
        let preprocessing = SurgePreprocessing::preprocess();
        let generators = HyperKZG::<_, KeccakTranscript>::setup(&[CommitShape::new(
            M,
            BatchType::SurgeReadWrite,
        ...
        // Call the prove_batch_test function with our ops.
        let (proof, debug_info) = SurgeProof::<
            Fr,
            HyperKZG<Bn254, KeccakTranscript>,
            MessageOp, // Now using MessageOp instead of XORInstruction.
            C,
            M,
            KeccakTranscript,
        ...
        SurgeProof::verify(&preprocessing, &generators, proof, debug_info)
            .expect("Proof verification should work");
    }
